...
The global CLAUDE.md mentions an `unzoo` browser. The `mcp__unzoo__*` **MCP tools
...
1. **Pick mode.**
   - Full scan (default): `bash .claude/skills/competitor-feedback/scan.sh`
   - Subscribe / incremental (only issues opened since last run):
     `bash .claude/skills/competitor-feedback/scan.sh new`
