...
# Cargo.toml, commits, tags, and pushes. GitHub Actions takes over from there.
#
...
if [ $# -lt 1 ]; then
  ...
  echo "Example: $0 0.2.0 (upstream) or $0 4.11.21-pe.1 (PE)" >&2
  exit 1
fi

VERSION="$1"
if [[ ! "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-pe\.[0-9]+)?$ ]]; then
  echo "ERROR: version must be semver, e.g. 0.2.0 or 4.11.21-pe.1" >&2
  exit 1
fi
...
echo "    https://github.com/jukejeew/solomd-pe/actions"
echo ""
echo "After all platforms finish (~15-20 min), open:"
echo "    https://github.com/jukejeew/solomd-pe/releases"
